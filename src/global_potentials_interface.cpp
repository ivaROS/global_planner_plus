...
//   }
  ...
  bool GlobalPotentialsInterface::updateTransform(const tf::StampedTransform& transform)
  {
    ...
    
    {
      ...
      if(msg)
      {
        if(msg != current_potentials_msg_)
        {
          costmap_.update(msg);
          current_potentials_msg_ = msg;
          ROS_INFO_STREAM_NAMED(name_ + ".update","Updating costmap potentials");
        }
        else
        {
          ROS_DEBUG_STREAM_NAMED(name_ + ".update", "Same potentials, no need to update");
        }
        return true;
      }
      else
  ...
  std_msgs::Header GlobalPotentialsInterface::getHeader() const
  {
    ...
    {
      ROS_WARN_NAMED(name_, "No potentials received, unable to return header!");
    }
